...
local ok, reason = xpcall(function()
  ...
  assert(not batch.valid(), "returning to a tab cannot revive its old confirmation")
  batch = assert(handle:prepare("approve", true))
  local frozen_panel = vim.api.nvim_get_current_buf()
  local temporary = vim.api.nvim_create_buf(false, true)
  vim.api.nvim_set_current_buf(temporary)
  vim.api.nvim_set_current_buf(frozen_panel)
  assert(not batch.valid(), "restoring a repurposed review window cannot revive its confirmation")
