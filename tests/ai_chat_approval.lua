...
local ok, reason = xpcall(function()
  ...
  vim.cmd("NvimAIChatApprove")
  f.phase("review")
  vim.fn.writefile({ "external change" }, f.files[2])
  vim.cmd("NvimAIChatApprove")
  f.phase("failed")
  states({ "accepted", "blocked", "blocked" })
  assert(f.snapshot().recovery_required and not f.snapshot().retry_safe)
  close()
  assert(f.snapshot().recovery_required, "closing cannot erase publication recovery")
  states({ "accepted", "blocked", "blocked" })
  assert(f.disk(1) == "proposed edit" and f.disk(2) == "external change")
  assert(f.disk(3) == "original text")
  print("ok - close after a blocked writer preserves accepted bytes and recovery evidence")
