...
- [ ] Hash and copy the explicit file allowlist from disk; reject symlinks and
  preserve modes. Record the manifest outside the public tree.
- [ ] Run existing tests in the copied tree with a disposable HOME/XDG environment,
...
- [ ] Derive test roots from each test's own file path in conversation-driver,
...
- [ ] Add the runner with explicit suite discovery, per-suite timeouts and logs,
...
- [ ] Add setup tests using real Neovim commands/mappings and isolated directories:
  ```lua
...
- [ ] Run those tests before implementation; expect missing facade and old-default failures.
- [ ] Implement the small facade, propagate one keymap policy to both setup paths,
  replace the personal redraw hook, and use command names in notices.
- [ ] Rename state/cache/runtime and tmux ownership namespaces. Make project-pair
  routing conditional on `tmux_project_pairs = true`; keep its isolated opt-in test.
- [ ] Run `python3 -I -B tests/run.py draft_setup ai_status ai_health ai_identity ai_transport ai_project_pairs ai_opencode_runtime_paths ai_runtime ai_staged`.
...
- [ ] Keep the existing poll/cancel fixture and regression unchanged.
- [ ] Add real-worker tests that call `receive("missing")` and `poll("missing")`
...
- [ ] Validate liveness and a pending request before reading its deadline, keeping
  polling slices distinct from request deadlines.
- [ ] Rerun the worker, conversation-store/lifetime, and editor conversation suites.
...
- [ ] Write portable installation/setup examples and native/staged walkthroughs.
...
- [ ] Run the full provider-free suite and a relocated-install smoke check from
  an unrelated cwd with spaces in the install path. Generate/verify help tags.
- [ ] Inspect every selected file for provenance, personal paths, private issue
  references, credentials, caches and logs. Compare source hashes again.
- [ ] Request one independent whole-change review against the spec and review focus;
